...
class Worker {
    constructor(link) {
        ...
        this.hashId = [];
        this.pageLink = link;
        this.teleportalApiUrl = 'https://vcms-api2.starlight.digital/player-api/{{hash}}?referer={{referer}}&lang=ua';
    }

    get parseOrUploadKeyboard() {
        return [[
            {text: `Parse (add to playlist)`, callback_data: stepEnum.WORKER_PARSE},
    ...
    _decodeUrlFromText = href => [...decodeURIComponent(href).matchAll(/file=(http.+?m3u8)?\?/g)];
    ...
    getHtml(link) {
        return this._loadPage(link).pipe(switchMap(data => fromPromise(data.text())));
    }

    getJson(link) {
        return this._loadPage(link).pipe(switchMap(data => fromPromise(data.json())));
    }
        ...
        return this.getHtml(this.pageLink).pipe(
            switchMap((data) => {
                ...
                if (!this.hashId.length) {
                    const link = this.pageLink.replace('teleportal.ua', 'tp-back.starlight.digital')
